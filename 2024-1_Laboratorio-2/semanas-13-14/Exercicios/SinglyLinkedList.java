public class SinglyLinkedList<E> implements List<E> {
	...
	public SinglyLinkedList<E> split(int divisor)throws IndexOutOfBoundsException{
		if (divisor < 0  ||  divisor >= numElements)
			throw new IndexOutOfBoundsException();

		SinglyLinkedList<E> firstHalfList = new SinglyLinkedList<E>();

		Node<E> current = head;
		for (int i = 0; i < divisor; i++){
			firstHalfList.insertLast(current.getElement());
			current = current.getNext();
		}
